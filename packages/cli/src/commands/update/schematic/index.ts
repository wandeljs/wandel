...
interface JsonSchemaForNpmPackageJsonFiles
  extends Manifest,
    WandelModuleManifestProperties {
...
function _validateForwardPeerDependencies(
  ...
  for (const [peer, range] of Object.entries(peers)) {
    ...
    if (
      !semver.satisfies(peerVersion, range, {
      ...
      logger.error(
        [
          `Package ${JSON.stringify(
            name
          )} has an incompatible peer dependency to`,
...
function _validateReversePeerDependencies(
  ...
  for (const [installed, installedInfo] of infoMap.entries()) {
    ...
    const peers = (installedInfo.target || installedInfo.installed).packageJson
      .peerDependencies;
    ...
    for (const [peer, range] of Object.entries(peers || {})) {
      ...
      if (
        !semver.satisfies(version, extendedRange, {
          includePrerelease: next || undefined,
        })
      ) {
        logger.error(
...
function _validateUpdatePackages(
  ...
  infoMap.forEach((info) => {
    ...
    peerErrors =
      _validateReversePeerDependencies(
        name,
        target.version,
        infoMap,
        pkgLogger,
  ...
  toInstall.forEach(([name, target, installed]) => {
    logger.info(
      `Updating package.json with dependency ${name} ` +
        `@ ${JSON.stringify(target.version)} (was ${JSON.stringify(
          installed.version
        )})...`
    );
...
function _getUpdateMetadata(
  ...
  const metadata = packageJson['wandel'];
  ...
  if (metadata['packageGroup']) {
    ...
    if (
      ...
      packageGroup.every((x) => typeof x == 'string')
    ) {
  ...
  if (metadata['requirements']) {
    ...
    if (
      ...
      Object.keys(requirements).some(
        (name) => typeof requirements[name] != 'string'
      )
    ) {
      logger.warn(
  ...
  if (metadata['migrations']) {
    ...
    if (typeof migrations != 'string') {
      logger.warn(
        `migrations metadata of package ${packageJson.name} is malformed. Ignoring.`
      );
    } else {
  ...
  const packagesToUpdate = [...infoMap.entries()]
    .map(([name, info]) => {
      ...
      if (
        ...
        const installedMajorVersion = semver.parse(
    ...
    .filter(
      ({ info, version, target }) =>
        target?.['ng-update'] &&
        semver.compare(info.installed.version, version) < 0
    )
    .map(({ name, info, version, tag, target }) => {
      // Look for packageGroup.
      const packageGroup = target['wandel']?.['packageGroup'];
      if (packageGroup) {
        ...
        const packageGroupName =
          target['wandel']?.['packageGroupName'] || packageGroupNames[0];
        if (packageGroupName) {
          if (packageGroups.has(name)) {
            return null;
          }

          packageGroupNames.forEach((x: string) =>
            packageGroups.set(x, packageGroupName)
          );
...
function _buildPackageInfo(
  ...
  for (const [version, { deprecated }] of Object.entries(
  ...
  const findSatisfyingVersion = (
    ...
    ((semver.maxSatisfying(packageVersionsNonDeprecated, targetVersion) ??
      semver.maxSatisfying(
        packageVersionsDeprecated,
        targetVersion
      )) as VersionRange | null) ?? undefined;
  ...
  const installedPackageJson =
    npmPackageJson.versions[installedVersion] || pkgJsonExists;
  ...
  if (targetVersion) {
    if (npmPackageJson['dist-tags'][targetVersion]) {
      targetVersion = npmPackageJson['dist-tags'][
        targetVersion
      ] as VersionRange;
  ...
  if (targetVersion && semver.lte(targetVersion, installedVersion)) {
    logger.debug(
      `Package ${name} already satisfied by package.json (${packageJsonRange}).`
    );
  ...
  const target: PackageVersionInfo | undefined = targetVersion
    ? {
        ...
        updateMetadata: _getUpdateMetadata(
          npmPackageJson.versions[targetVersion],
          logger
        ),
      }
    : undefined;
...
function _buildPackageList(
  ...
  for (const pkg of commandLinePackages) {
    ...
    if (!m) {
      logger.warn(
        `Invalid package argument: ${JSON.stringify(pkg)}. Skipping.`
      );
      continue;
    }
    ...
    if (!version) {
      logger.warn(
        `Package not installed: ${JSON.stringify(npmName)}. Skipping.`
      );
      continue;
    }
    ...
    packages.set(
      npmName,
      (maybeVersion || (options.next ? 'next' : 'latest')) as VersionRange
    );
  }
...
function _addPackageGroup(
  ...
  const metamorphisUpdateMetadata = npmPackageJson.versions[version]['wandel'];
  if (!metamorphisUpdateMetadata) {
    return;
  }

  const packageGroup = metamorphisUpdateMetadata['packageGroup'];
  if (!packageGroup) {
    return;
  }
  ...
  if (
    Array.isArray(packageGroup) &&
    !packageGroup.some((x) => typeof x != 'string')
  ) {
...
function _addPeerDependencies(
  ...
  for (const [peer, range] of Object.entries(
    ...
    if (peerPackageJson) {
      const peerInfo = _buildPackageInfo(
        tree,
        packages,
        allDependencies,
        peerPackageJson,
...
function _getAllDependencies(
  tree: Tree
): Array<readonly [string, VersionRange]> {
  const { dependencies, devDependencies, peerDependencies } = tree.readJson(
    '/package.json'
  ) as JsonSchemaForNpmPackageJsonFiles;

  return [
    ...(Object.entries(peerDependencies || {}) as Array<
      [string, VersionRange]
    >),
...
function _formatVersion(version: string | undefined) {
  ...
  if (!semver.valid(version)) {
    throw new SchematicsException(
      `Invalid migration version: ${JSON.stringify(version)}`
    );
  }
    ...
    const npmPackageJsonMap = allPackageMetadata.reduce(
      (acc, npmPackageJson) => {
        ...
        if (!npmPackageJson.name) {
          if (
            npmPackageJson.requestedName &&
            packages.has(npmPackageJson.requestedName)
          ) {
            throw new SchematicsException(
              `Package ${JSON.stringify(
                npmPackageJson.requestedName
