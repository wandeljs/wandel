...
(function (t) {
	...
	(t.exports = u),
		(u.eastAsianWidth = function (e) {
			var s2 = e.charCodeAt(0),
				C = e.length == 2 ? e.charCodeAt(1) : 0,
				D = s2;
			return (
				55296 <= s2 &&
					s2 <= 56319 &&
					56320 <= C &&
					C <= 57343 &&
					((s2 &= 1023), (C &= 1023), (D = (s2 << 10) | C), (D += 65536)),
...
function c(t, u = {}) {
	if (
		typeof t != "string" ||
		t.length === 0 ||
		((u = { ambiguousIsNarrow: true, ...u }), (t = $(t)), t.length === 0)
	) {
		return 0;
	}
	t = t.replace(Y(), "  ");
...
function U() {
	...
	for (const [u, F] of Object.entries(r)) {
		for (const [e, s2] of Object.entries(F)) {
			(r[e] = { open: `\x1B[${s2[0]}m`, close: `\x1B[${s2[1]}m` }),
				(F[e] = r[e]),
				t.set(s2[0], s2[1]);
		}
	...
	return (
		...
		(r.color.ansi16m = T()),
		(r.bgColor.ansi = L(v)),
		(r.bgColor.ansi256 = M(v)),
		(r.bgColor.ansi16m = T(v)),
		Object.defineProperties(r, {
			...
			ansi256ToAnsi: {
				value: (u) => {
					...
					let D =
						30 + ((Math.round(s2) << 2) | (Math.round(e) << 1) | Math.round(F));
					return C === 2 && (D += 60), D;
				},
				enumerable: false,
			},
		...
		if (
			(E !== 0 &&
				(n >= u &&
					...
					(i.push(""), (n = 0)),
				(n > 0 || F.trim === false) && ((i[i.length - 1] += " "), n++)),
			F.hard && D[E] > u)
		) {
			const B = u - n,
				A = 1 + Math.floor((D[E] - B - 1) / u);
...
var h = class {
	constructor(
		{ render: u, input: F = stdin, output: e = stdout, ...s2 },
		C = true,
	) {
		(this._track = false),
			(this._cursor = 0),
	...
	prompt() {
		...
		return (
			...
			new Promise((F, e) => {
				this.once("submit", () => {
					this.output.write(src.cursor.show),
	...
	onKeypress(u, F) {
		if (
			...
			F?.name &&
				!this._track &&
				V.has(F.name) &&
				this.emit("cursor", V.get(F.name)),
			F?.name && tD.has(F.name) && this.emit("cursor", F.name),
			u &&
			...
			if (this.opts.validate) {
				...
				e &&
					((this.error = e), (this.state = "error"), this.rl.write(this.value));
			}
...
`),
	...
	restoreCursor() {
		const u =
			P(this._prevFrame, process.stdout.columns, { hard: true }).split(`
`).length - 1;
		...
		if (u !== this._prevFrame) {
			if (this.state === "initial") {
				...
				if ((this.restoreCursor(), F && F?.length === 1)) {
					const e = F[0];
					this.output.write(src.cursor.move(0, e)),
						this.output.write(src.erase.lines(1));
					const s2 = u.split(`
`);
					this.output.write(s2[e]),
						(this._prevFrame = u),
						this.output.write(src.cursor.move(0, s2.length - e - 1));
					return;
...
var sD = class extends h {
	...
	constructor(u) {
		super(u, false),
			...
			this.on("confirm", (F) => {
				this.output.write(src.cursor.move(0, -1)),
					(this.value = F),
					(this.state = "submit"),
					this.close();
			}),
...
export { prompt };
//# sourceMappingURL=prompt-OO2NJXBJ.js.map
